...
// ── footer manifesto ─────────────────────────────────────────────────────────
// A plain, quiet paragraph — the write-in fill was removed on purpose (at rest
// mid-fill it read as broken, not as motion). Soft cream, regular weight.
const FOOT_LINE =
...
function FooterManifesto() {
  return (
    <p className="mt-5 max-w-[34ch] text-pretty font-satoshi text-[0.95rem] leading-relaxed text-cream/60">
      {fr(FOOT_LINE)}
    </p>
  );
}
...
function FooterCol({
  ...
  return (
    <div className="flex flex-col items-start gap-3.5">
      <p className="font-satoshi text-[0.8rem] font-bold tracking-tight text-cream/55">
        {title}
      </p>
      {children}
    </div>
